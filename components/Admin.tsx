      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 pb-8 border-b border-white/5">
          <div>
            ...
            <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest">
              {data.length} Total Messages • {data.filter(d => d.status === 'pending').length} Pending Review
            </p>
          </div>
          <button 
        ...
        <div className="space-y-2">
          {data.length === 0 ? (
            ...
            data.map((item) => (
              <div key={item.id} className="bg-[#0D0D12] border border-white/5 p-6 rounded-xl flex flex-col md:flex-row gap-8 items-center justify-between hover:bg-[#111118] transition-colors">
                ...
                <div className="flex items-center gap-6 w-full md:w-1/4">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <h3 className="text-sm font-black text-white truncate uppercase">{item.fullName}</h3>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-10 w-full md:w-2/4 text-left">
                  ...
                  <div>
                    <p className="text-[7px] font-black text-gray-700 uppercase tracking-widest mb-1">Commitment</p>
                    <p className="text-[10px] font-black text-white">{item.amount ? `₦${item.amount}` : 'N/A'}</p>
                  </div>
                  <div className="hidden sm:block">
                    <p className="text-[7px] font-black text-gray-700 uppercase tracking-widest mb-1">Created At</p>
