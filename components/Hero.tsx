      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex -space-x-3" aria-label="Supporters avatars">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="w-12 h-12 rounded-full border-2 border-[#0B0B0F] bg-[#1A1A24] flex items-center justify-center overflow-hidden">
                   <img src={`https://picsum.photos/seed/supporter-${i}/100/100`} alt={`Vidavest Supporter ${i}`} className="w-full h-full object-cover grayscale opacity-80" />
                </div>
              ))}
            </div>
        ...
        <div className="hidden lg:block relative">
           <div className="relative z-10 w-full aspect-square rounded-[3rem] overflow-hidden border border-white/5 shadow-2xl">
              <img 
                src="https://picsum.photos/seed/vidavest-hero-impact/800/800" 
              ...
              <div className="absolute bottom-10 left-10 right-10 p-8 bg-black/40 backdrop-blur-md rounded-3xl border border-white/10">
                 <blockquote className="text-sm font-medium italic text-gray-300">"We are committed to bridging the gap between talent and opportunity, fostering economic stability and youth inclusion across Nigeria."</blockquote>
                 <p className="mt-4 text-[--gold] font-black text-xs uppercase tracking-widest">— Vidavest Executive Board</p>
              </div>
           </div>
           {/* Floating element */}
           <div className="absolute -top-10 -right-10 bg-[#14141C] p-6 rounded-2xl border border-[#2A2A38] shadow-2xl animate-float" aria-hidden="true">
