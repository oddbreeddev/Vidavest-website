...
const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  return (
    <section className="relative pt-48 pb-32 px-6 overflow-hidden min-h-[90vh] flex items-center">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-900/10 rounded-full blur-[120px] -mr-48 -mt-48"></div>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-amber-500/5 rounded-full blur-[120px] -ml-48 -mb-48"></div>
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10 grid lg:grid-cols-2 gap-16 items-center">
        <div className="text-left">
          ...
          <p className="text-xl text-gray-400 mb-12 max-w-xl leading-relaxed font-medium">
            Vidavest is a purpose-driven investment platform designed to fund, mentor, and scale the next generation of African entrepreneurs.
          </p>
            ...
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="w-12 h-12 rounded-full border-2 border-[#0B0B0F] bg-[#1A1A24] flex items-center justify-center overflow-hidden">
                   <img src={`https://picsum.photos/seed/${i}/100/100`} alt="Supporter" className="w-full h-full object-cover grayscale opacity-80" />
                </div>
              ))}
            </div>
        ...
        <div className="hidden lg:block relative">
           <div className="relative z-10 w-full aspect-square rounded-[3rem] overflow-hidden border border-white/5 shadow-2xl">
              ...
              <div className="absolute bottom-10 left-10 right-10 p-8 bg-black/40 backdrop-blur-md rounded-3xl border border-white/10">
                 <p className="text-sm font-medium italic text-gray-300">"Vidavest is more than a platform; it's a movement for economic stability and youth inclusion."</p>
                 <p className="mt-4 text-[--gold] font-black text-xs uppercase tracking-widest">— Charity Aminu Kwala</p>
              </div>
           </div>
           {/* Floating element */}
           <div className="absolute -top-10 -right-10 bg-[#14141C] p-6 rounded-2xl border border-[#2A2A38] shadow-2xl animate-bounce duration-3000">
